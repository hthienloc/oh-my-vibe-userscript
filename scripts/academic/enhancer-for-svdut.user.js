...
// @grant        none
// @license      MIT
// @downloadURL  https://github.com/hthienloc/enhancer-for-svdut/raw/main/enhancer-for-svdut.user.js
// @updateURL    https://github.com/hthienloc/enhancer-for-svdut/raw/main/enhancer-for-svdut.user.js
// ==/UserScript==

(function() {
    'use strict';

    // 0. Hide annoying slideshow
    const style = document.createElement('style');
    style.textContent = '.defaultSlideShow { display: none !important; }';
    ...
    function init() {
        ...
        // --- 0. Global Navbar Logic ---
        const loginLink = document.getElementById('linkDangNhap');
        if (loginLink && savedData) {
            // Add flag to trigger auto-login when clicking this specific link
        ...
        if (url.includes('PageDangNhap.aspx')) {
            ...
            if (accInput && passInput && loginBtn && !document.getElementById('remember-me')) {
                // 1.1. Create the "Remember Login" row
        ...
        if (url.includes('PageCaNhan.aspx')) {
            ...
            if (tabs && !document.getElementById('privacy-toggle-btn')) {
                ...
                btn.style.height = 'auto'; // Prevent height constraints
            ...
            if (targetParent) {
                ...
                // Fetch Home Page for Original Notifications
                const fetchHome = fetch('/Default.aspx')
                    .then(r => r.text());

                // Fetch Exam Schedule
                const fetchExams = fetch('/PageLichTH.aspx')
                    .then(r => r.text());
                ...
                Promise.all([fetchHome, fetchExams])
                    .then(([homeHtml, examPageHtml]) => {
                        ...
                        // 1. Process Original Notifications from Home Page
                        const homeParser = new DOMParser();
                        const homeDoc = homeParser.parseFromString(homeHtml, 'text/html');
                        
                        // Extract the "Thông báo" search box and the tabs
                        ...
                        if (searchBox || tabsBox) {
                            // Strip JQuery UI classes to avoid conflicts during re-initialization
                            const cleanHTML = (html) => {
                                return html.replace(/ui-tabs-active|ui-state-active|ui-state-hover|ui-state-focus/g, '');
                            };
                                ...
                                if (typeof window.jQuery !== 'undefined' && window.jQuery.fn.tabs) {
                                    ...
                                    $tabs.tabs({
                                        activate: function(event, ui) {
                                            ...
                                            // Check if already loaded or if it's the first tab (which is already in the homeHtml)
                                        ...
                                        create: function(event, ui) {
                                            // Mark the first panel as loaded initially
        ...
        // --- 4. LMS Logic ---
        if (url.includes('lms.dut.udn.vn')) {
            ...
            if (loginForm && (url.includes('/login/index.php') || url.includes('/login/'))) {
                ...
                if (accInput && passInput && loginBtn && !document.getElementById('remember-me-lms')) {
                    // 4.1.1. Create the "Remember Login" checkbox
                    ...
                    // 4.1.2. Load saved credentials & Auto-fill
                    if (lmsSavedData) {
                        accInput.value = lmsSavedData.acc;
                        passInput.value = lmsSavedData.pass;
                        rememberCb.checked = true;

                        // 4.1.3. Fast Login Trigger
                        if (url.includes('autologin=true')) {
                            setTimeout(() => loginBtn.click(), 500);
                        }
                    }

                    // 4.1.4. Save credentials on submit
            ...
            // 4.2. Global Login Link Logic (Modify the header link)
            const lmsLoginLink = document.querySelector('.login a[href*="/login/index.php"]');
            if (lmsLoginLink && lmsSavedData) {
    ...
    function exportScheduleToGoogleCalendar() {
        ...
        const reminderMinutes = parseInt(docAll.getElementById('export-reminder-select')?.value) || 30;
